    ...
    namespace INTERNAL_CATCH_MAKE_NAMESPACE(TestName) {                       \
        ...
        static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {       \
            using TestInit = decltype(                                        \
                create<TestName, INTERNAL_CATCH_REMOVE_PARENS(TmplTypes)>(    \
...
#define INTERNAL_CATCH_TEMPLATE_LIST_TEST_CASE_2(TestName, TestFunc, Name, \
    ...
    namespace INTERNAL_CATCH_MAKE_NAMESPACE(TestName) {                    \
        ...
        static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {    \
            using TestInit =                                               \
                decltype(convert<TestName>(std::declval<TmplList>()));     \
            TestInit t;                                                    \
            t.reg_tests();                                                 \
            return 0;                                                      \
        }();                                                               \
...
#define INTERNAL_CATCH_TEMPLATE_LIST_TEST_CASE_METHOD_2(                    \
    ...
    namespace INTERNAL_CATCH_MAKE_NAMESPACE(TestName) {                     \
        ...
        static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {     \
            using TestInit =                                                \
                decltype(convert<TestNameClass>(std::declval<TmplList>())); \
            TestInit t;                                                     \
            t.reg_tests();                                                  \
            return 0;                                                       \
        }();                                                                \
...
std::make_shared<T>(token, m_substring);
if (m_exclusion)
