...
public class StandardPlayer implements PlayerStrategy {
	...
	public Card playCard() {
		...
		if ((indexOfCard2Play = findRank(rank2Match)) >= 0) {
			return cards.remove(indexOfCard2Play);
		} else {
			return cards.remove(findSuit(suit2Match));
		}
	}
	...
	public Card.Suit declareSuit() {
		return Card.Suit.values()[(int)(Math.random() * Card.Suit.values().length)];
	}
